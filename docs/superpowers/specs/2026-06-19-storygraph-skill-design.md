...
这个图谱必须服务后续动态发现到的世界观参考模板集合的抽取需求。阶段 1 暂不生成最终 Markdown 调研文档，但它生成的节点、边、事件、证据索引和覆盖账本，必须能支撑阶段 2 按模板生成尽可能完整的文档。当前 CultivationWorld 样例目录发现到 37 个模板；这个数量只是样例事实和默认集成检查值，不是写死逻辑契约。
...
- 该模板目录当前实际有 37 个 `*模板.md` 文件；该数值只描述 CultivationWorld 样例现状。
- 模板 README 清单还提到两个不存在的模板文件：
  - `出门游历流程分析模板.md`
  - `时间行动与事件耗时模板.md`
- 阶段 1 的范围以动态发现到的实际存在模板文件为准；README 中列出但文件不存在的模板记录为 `missing_template_file` 警告，不阻塞建图。后续文件补齐后，模板扫描会自动把它们纳入下一次需求矩阵。
...
6. Codex 主 agent 调度模板需求分析 agent 读取动态发现到的模板文件。模板需求分析 agent 提炼字段、表格、卡片、案例、证据要求、图节点映射、事件映射、关系映射和 readiness 规则；Python 只校验 schema、归一化 ID 并落盘：

```text
requirements\template-requirements.json
```

7. 生成动态模板需求矩阵。矩阵必须逐模板记录字段、表格、卡片、案例、证据和缺口判定规则，并把每项需求映射到图节点类型、事件类型、关系类型和证据类型。
...
11. Codex 主 agent 调度审查 agent：
   ...
   - template readiness reviewer：检查动态发现到的模板需求项是否有候选支撑或明确缺口。
...
- `template-readiness.json` 覆盖动态发现到的全部模板，说明每个模板的数据准备情况。
- 图内容能支撑动态发现到的全部模板后续抽取，而不是通用概念图。每个模板需求项必须被标记为 `covered`、`needs_review` 或 `not_found_in_source`，不得缺项。
...
| 模板发现 | 实际存在的 `*模板.md` 由模板发现动态决定；当前 CultivationWorld 样例默认集成检查值为 37，README 缺失模板只记警告 |
...
| 模板 readiness | 动态发现到的每个模板都有 `requirement_statuses`，每项状态非空 |
...
| 检查项 | 通过标准 |
|--------|----------|
| 抽取任务数 | 动态发现到的每个模板都有 `extractions/<模板名>.json` 或失败记录 |
...
- template readiness reviewer：检查动态发现到的模板需求是否有候选支撑、缺口状态和证据链。
...
- 最终审查 agent：检查动态发现到的模板文档整体一致性和覆盖性。
...
这些值是可移植默认配置，不写入本机绝对路径。本机路径写入不提交的 `storygraph.local.json` 或命令参数，例如模板目录和 graphify 仓库位置。`target_count: 1000` 和 `element_lanes` 只是默认示例，逻辑必须读取配置；不同小说、不同模板质量或抽取效果不好时，可以通过 override 调整 chunk 大小、目标 chunk 数、lane 列表、并发、batch 和 repair 策略。`require_all_templates_scope` 表示“全部模板”以实际存在的模板文件为准；README 索引缺文件按 `readme_missing_policy` 记警告。`readiness_warning_threshold` 只用于提示薄弱模板；阶段 1 的硬性验收是动态发现到的模板均有需求矩阵、全文分块均被扫描、每个必要 lane 有 agent 输出或结构化失败记录、每个需求项都有覆盖状态。当前 CultivationWorld 样例默认集成检查值可配置为 37，但实现逻辑不得写死该数量。
...
- 模板需求提炼策略，包括可配置 agent role、每包 1-5 个模板、manual override 和 Python validate-only。
...
10. 实现模板需求分析 agent 流程，生成动态模板需求矩阵；Python 只做 schema 校验和落盘。
...
15. 用 `凡人修仙传.txt` 做阶段 1 验证，重点验证 chunk/lane/agent ledger、review findings、canonical graph 来源和动态模板 readiness。
...
- 首版按动态发现到的实际存在模板执行；当前 CultivationWorld 样例发现到 37 个模板，README 提到但文件缺失的两个模板只记录警告，不阻塞阶段 1。

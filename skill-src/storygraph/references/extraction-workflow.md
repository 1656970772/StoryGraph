...
Stage 2 is an agent-driven template document generation workflow. Python prepares one task packet per template document, validates records, tracks ledgers, enforces output policy, and renders Markdown. Stage 2 agents produce the semantic document content as `stage2-extraction-record.v1` JSON records.
...
The default Stage 2 output policy is draft-first. `draft` writes to `<graph_dir>/<stage2_output_policy.default_dir>/<template_name>.md` and does not overwrite an existing formal Markdown document beside the novel. `backup-and-overwrite` writes the formal document beside the novel, first copying an existing formal Markdown file to the configured `.bak` path. `merge` remains fail-closed until a separate merge contract exists.

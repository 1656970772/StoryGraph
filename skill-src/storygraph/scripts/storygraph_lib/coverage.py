...
DEFAULT_CHAPTER_HEADING_PATTERNS = [
    r"^第\s*[0-9０-９一二三四五六七八九十百千万零〇两]+\s*[章节回卷集部篇].*",
    r"^(序章|楔子|终章|尾声|后记|番外).*",
    r"^Chapter\s+\d+.*",
]
...
def make_chunk_ledger(
    ...
    patterns = active_strategy.get("chapter_heading_patterns") or DEFAULT_CHAPTER_HEADING_PATTERNS
...
def _chapter_sections(text: str, patterns: list[str]) -> list[tuple[int, int, str | None]]:
    ...
    for line in text.splitlines(keepends=True):
        ...
        if _matches_heading(normalized_heading, patterns):
            headings.append((offset, normalized_heading))
        offset += len(line)
    if not headings:
        return []
...
def _matches_heading(heading: str, patterns: list[str]) -> bool:
    return bool(heading) and any(re.match(pattern, heading) for pattern in patterns)
